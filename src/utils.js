...
export const getAngleBase = (engine) => {
  ...
  // Get v1, v2, v3 values from window.gameSettings or use default if not set
  const v1 = (window.gameSettings && window.gameSettings.v1) || 30;
  const v2 = (window.gameSettings && window.gameSettings.v2) || 50;
  const v3 = (window.gameSettings && window.gameSettings.v3) || 60;
...
export const addScore = (engine, isPerfect) => {
  ...
  const score = lastGameScore + (successScore || 250) + ((perfectScore || 250) * perfect)
